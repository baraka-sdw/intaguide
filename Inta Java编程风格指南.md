...
除了行结束符序列，ASCII水平空格字符(0x20，即空格)和水平制表符(0x09，即`tab`)是源文件中仅允许出现的空白字符，这意味着：
...
### 4.2 块缩进：4个空格长的`tab`制表符 ###

每当开始一个新的块，缩进增加4个空格长的`tab`制表符，当块结束时，缩进返回先前的缩进级别。缩进级别适用于代码和注释。(见4.1.2节中的代码示例)
...
#### 4.5.2 自动换行时缩进至少+2个`tab`制表符 ####

自动换行时，第一行后的每一行至少比第一行多缩进2个`tab`制表符。

当存在连续自动换行时，缩进可能会多缩进不只2个制表符(语法元素存在多级时)。一般而言，两个连续行使用相同的缩进当且仅当它们开始于同级语法元素。
